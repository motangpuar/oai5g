                                             ...
                                             [dlsch->dlsch_config.number_rbs * NR_NB_SC_PER_RB])
{
  ...
  const int rx_llr_size =
      nr_get_G(dlsch->dlsch_config.number_rbs, s1, nb_re_dmrs, dmrs_len, dlsch->dlsch_config.qamModOrder, dlsch->Nl);
                            ...
                            int16_t llr[NR_MAX_NB_LAYERS > 4 ? 2 : 1][llrSize])
{
  ...
  const uint8_t nb_re_dmrs = (dmrs_type == NFAPI_NR_DMRS_TYPE1) ? 6 * dlsch[0].dlsch_config.n_dmrs_cdm_groups
