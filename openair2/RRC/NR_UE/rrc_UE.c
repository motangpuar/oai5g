...
static void nr_rrc_handle_msg3_indication(NR_UE_RRC_INST_t *rrc, rnti_t rnti, int gnb_id)
{
  ...
  switch (rrc->ra_trigger) {
    ...
    case RRC_CONNECTION_REESTABLISHMENT:
      ...
      nr_rlc_reestablish_default_srb(rrc->ue_id, srb_id);
      ...
      rrcNB->Srb[srb_id] = RB_ESTABLISHED;
      break;
...
void *rrc_nrue(void *notUsed)
{
  ...
  case NR_RRC_MAC_MSG3_IND:
    nr_rrc_handle_msg3_indication(rrc, NR_RRC_MAC_MSG3_IND(msg_p).rnti, NR_RRC_MAC_MSG3_IND(msg_p).gnb_id);
    break;
