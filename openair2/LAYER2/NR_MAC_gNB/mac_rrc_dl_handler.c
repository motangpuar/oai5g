...
void f1_setup_response(const f1ap_setup_resp_t *resp)
{
  ...
  AssertFatal(du_cell->nr_cellid == cu_cell->nr_cellid,
              "CellID mismatch: DU %ld vs CU %ld\n",
              du_cell->nr_cellid,
              cu_cell->nr_cellid);
...
void dl_rrc_message_transfer(const f1ap_dl_rrc_message_t *dl_rrc)
{
  ...
  if (dl_rrc->old_gNB_DU_ue_id != NULL) {
    AssertFatal(*dl_rrc->old_gNB_DU_ue_id != dl_rrc->gNB_DU_ue_id, "logic bug: current and old gNB DU UE ID cannot be the same\n");
